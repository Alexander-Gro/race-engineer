...
/**
 * Highest event tier each level will *speak*. `off` = nothing; `low` = urgent (Tier 1);
...
/**
 * Should this proactive call-out be spoken now? The proactivity level caps the tier, and during a
 * quiet window only urgent (Tier < 2) calls get through.
 */
