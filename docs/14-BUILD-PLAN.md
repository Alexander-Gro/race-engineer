...
The north star (CLAUDE.md): _every Tier-1+ utterance is generated by the AI from the real numbers and
the voice only speaks it; template is a **degraded fallback only**; only Tier-0 reflex spotter calls
("car left / 3-wide / clear") stay pre-rendered._
...
worth saying, gated so it isn't chatty. The only pre-rendered audio is the reflex spotter calls.
...
2. **Flip proactive phrasing `templatePhraser` → `llmPhraser` by default** in the voice layer
   (`EngineerVoice` / `ProactiveVoiceRouter`, `packages/radio`): Tier-1+ events are phrased by the LLM
   from the event payload + `RaceState`; `templatePhraser` becomes the *fallback* when no provider.
   Answers already route through the provider-aware brain — just ensure the default profile has a model.
...
5. **Keep Tier-0 reflex pre-rendered** — do **not** route "car left/3-wide/clear" through the LLM.
